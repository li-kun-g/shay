...
export default function CommentSheet(props: {
  ...
  async function onSend() {
    ...
    let optimisticId = `temp-${Math.random().toString(36).slice(2)}`;
    ...
    startTransition(async () => {
      try {
        const res = await createComment({ postId, content: text, anonymous: isAnon });
  ...
  const heightClass = showModerationMsg ? "h-[400px]" : "h-[78vh] sm:h-[88vh] max-h-[880px]";
          ...
          {showModerationMsg ? (
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500">
              ...
              <h3 className="text-xl font-bold">
                {t("composer.sentToModeration" as any) || "Your reply sent for moderation"}
              </h3>
              <p className="text-sm text-gray-500 mt-2 max-w-[280px]">
                {t("composer.moderationDesc" as any) || "It will appear in the thread once it has been approved by the team!"}
              </p>
              <button
                onClick={() => { setShowModerationMsg(false); setContent(""); }}
            ...
            <>
              ...
              <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto px-4 sm:px-6 pb-3">
                <List
                  loading={loading}
                  comments={comments}
                  myUserId={myUserId}
...
function List({ loading, comments, myUserId, onRequestDelete, deleting }: {
  loading: boolean;
  ...
  const { t } = useI18n();
  if (loading) return <div className="py-6 text-sm text-gray-500">{t("common.loading")}</div>;
  if (comments.length === 0) return <div className="py-10 text-center text-sm text-gray-400">☕ {t("comments.empty")}</div>;
  ...
  return (
    <div className="space-y-3 pt-4">
      {comments.map((c) => {
        ...
        return (
          <div key={c.id} className="rounded-2xl border bg-white p-3 shadow-sm dark:bg-[var(--surface)] dark:border-[var(--border-strong)]">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-sm">
                  <span>{c.author?.emoji ?? "☕"}</span>
                  <span className="font-semibold">{c.anonymous ? t("common.anonymous") : (c.author?.name || "Student")}</span>
                  {!c.anonymous && c.author?.username && (
                    <Link href={`/u/${c.author.username}`} className="text-xs text-gray-400 hover:underline">@{c.author.username}</Link>
                  )}
                </div>
                <p className="mt-1 break-words text-sm leading-relaxed">{c.content}</p>
              </div>
              {canDelete && (
                <button onClick={() => onRequestDelete(c.id)} disabled={deleting} className="shrink-0 rounded-full p-2 hover:bg-gray-50 opacity-60">🗑️</button>
              )}
            </div>
          </div>
        );
...
function Composer(props: {
  ...
  const { t } = useI18n();
  const { inputRef, status, content, setContent, anonymous, setAnonymous, canSend, isPending, onSend } = props;
  ...
  return (
    <div className="border-t border-gray-100 dark:border-white/5 pt-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        {status !== "authenticated" && <span className="text-xs text-gray-400">{t("comments.signInToReply")}</span>}
      </div>
