...
import SupportOpenCard from "./support-open-card";
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-6 md:max-w-2xl space-y-4">
      ...
      <section className="rounded-3xl border bg-white p-5 shadow-sm">
        <h1 className="text-xl font-semibold">{dict["settings.support.title"]}</h1>
        <p className="mt-1 text-sm text-gray-500">
          {dict["settings.support.pageDesc.before"]} <b>{dict["settings.support.pageDesc.bold"]}</b>{" "}
          {dict["settings.support.pageDesc.after"]}
        </p>
      </section>
